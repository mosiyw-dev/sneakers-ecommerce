...
export function TiltCard({ children, max = 12, glare = true, className }: TiltCardProps) {
  ...
  const glareBg = useMotionTemplate`radial-gradient(circle at ${gx}% ${gy}%, rgba(255, 255, 255, 0.15), transparent 60%)`;
    ...
    >
      ...
      {glare && enabled ? (
        <motion.div
          ...
          className="pointer-events-none absolute inset-0 z-20 opacity-80 mix-blend-overlay transition-opacity duration-300"
        />
      ) : null}
    </motion.div>
  );
